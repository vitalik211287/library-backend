...
export const getBookFromKnigoland = async (
  isbn: string,
  ...
  if (!firstBook.url) {
    throw new Error(
      `Knigoland book URL missing for ISBN ${isbn}`,
    );
  }
  ...
  if (!pageResponse.ok) {
    throw new Error(
      `Knigoland page failed: ${pageResponse.status}`,
    );
  }
  ...
  if (firstBook.picture) {
    try {
      ...
      console.error(
        "Knigoland cover upload failed:",
        error,
      );
    }
  }

  // 6. Єдиний ProviderBook
  return {
    isbn: pageIsbn ?? isbn,
